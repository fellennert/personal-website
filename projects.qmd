...
For a graduate Computational Social Science course, I compared different approaches to supervised text classification: dictionary-based (AFINN), bag-of-words (Naive Bayes with TF–IDF), fine-tuned BERT, and zero-shot prompting with a local LLM. Using 500 training examples from the IMDb reviews data set, the project highlights how preprocessing and model choice affect accuracy, precision, recall, F1, and runtime—with BERT and the LLM offering the best performance at higher compute cost. [READ MORE](./projects/ml_classifiers.qmd)